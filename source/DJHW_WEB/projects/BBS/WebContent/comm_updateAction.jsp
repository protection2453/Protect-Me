	...
	//==========댓글수정==========
		...
		if (!userID.equals(comm.getUserID())) {
			...
			if (request.getParameter("commTitle") == null || request.getParameter("commContent") == null || request.getParameter("commTitle").equals("") || request.getParameter("commContent").equals("")) {
				...
				int result = commDAO.update(commID, request.getParameter("commTitle") != null ? request.getParameter("commTitle") : null , request.getParameter("commContent") !=null ? request.getParameter("commContent") : null);
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
